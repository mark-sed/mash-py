...
class Initializer():
    ...
    def __init__(self, argparser):
        ...
        if "-e" not in sys.argv[1:mash_args_i]:
            self.mash_file = sys.argv[mash_args_i-1]
            self.opts = self.argparser.parse_args(sys.argv[1:mash_args_i-1])
        else:
        ...
        if self.code is None:
            if self.mash_file is None:
                # Stdin
                self.code = sys.stdin.read()
            else:
                # File passed in
                with open(self.mash_file, 'r', encoding='utf-8') as mash_file:
    ...
    def interpret(self):
        ...
        """
        interpreter.interpret(self.opts, self.code, self.mash_args)
    ...
    def add_arguments(self, argparser):
        ...
        argparser.add_argument('-l', '--lib-path', action='append', help='Path to folders to search for imports', 
                                required=False, default=None, dest='lib_path', nargs=1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        format_ex("'return' outside of a function.", initializer.code, initializer.mash_file)
    except mex.FlowControlBreak:
        format_ex("'break' outside of a loop.", initializer.code, initializer.mash_file)
    except mex.FlowControlContinue:
        format_ex("'continue' outside of a loop", initializer.code, initializer.mash_file)
    except mex.MashException as e:
        format_ex(str(e), initializer.code, initializer.mash_file)
    except LarkError as e:
        if getattr(e, "get_context", None) is not None:
            format_lark_ex(e, initializer.code, initializer.mash_file)
        else:
            # Chained
            format_ex(e.orig_exc, initializer.code, initializer.mash_file)